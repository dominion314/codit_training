...
def call(def version) {
    ...
    pipeline {
        ...
        stages {
            stage('Running Go Lint') {
                steps {
                    setBuildStatus("Running go lint validation", "PENDING")
                    sh("""
                ...
                post {
                    success {
                        setBuildStatus("Go Lint CI successful", "SUCCESS")
                    }
                    failure {
                        setBuildStatus("Go Lint CI failed", "FAILURE")
                    }
                }
            }
        }
