...
if config.key?('storageBuckets')
  ...
  storage_config.each do |bucket_name, bucket_def|
    next unless bucket_def.key?('default_object_acls')

    default_object_acls = bucket_def.fetch('default_object_acls')
    default_object_acls.each do |entity_type, entity_def|
      if entity_type == 'primitiveRoles'
        entity_def.each do |project_number, roles_def|
          roles_def.each do |member, role|
            ...
            control 'Storage Default Object Access Control Lists'
            describe google_storage_default_object_acl(bucket: bucket_name,
                                                       entity: entity) do
