"""
This is used to convert JSON firewall dumps into the appropriate YAML format for customer requests.
For CSV files you can use the following link to convert your data to JSON: https://csvjson.com/.
JSON formatter to correct any issues: https://www.freeformatter.com/json-formatter.html
"""
...
def main():
    ...
    with open("kohls-merch-xpn-lle.json", "r") as file:
        data = json.load(file)
    output = open("converter_output.yml", "w+")
    #print(data[0].Networks())
    vpc_name = data[0]["name"]
    master_data = {"vpcs": {vpc_name: {"firewallRules":{}}}}
    for item in data:
        ...
        if destination_ranges == "":
            destination_ranges.strip('"').strip("'")
        else:
        ...
        master_data["vpcs"][vpc_name]["firewallRules"][item["name"]] = structured_data
    sdata = yaml.dump(master_data, allow_unicode=True)
    output.write(sdata)
    # print(data)
